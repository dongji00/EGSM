...
from test_code.networks import ResnetConditionHR
...
gezi_test/input
-tb
gezi_test/background/0001.png
-b
gezi_test/bg.png'''

"""Parses arguments."""
parser = argparse.ArgumentParser(description='Background Matting.')
parser.add_argument('-m', '--trained_model', type=str, default='mine_train_0225_train_real',help='Trained background matting model')
...
#initialize network
#fo=glob.glob(model_main_dir + 'net_epoch_*')
fo=glob.glob(model_main_dir + 'net_epoch_*.pth')
print(fo)
model_name1=fo[0]
...
for i in range(0,len(test_imgs)):
	...
	if args.back is None:
		...
		bg_im0=cv2.imread(os.path.join(data_path, filename.replace('_img','_bg'))); bg_im0=cv2.cvtColor(bg_im0,cv2.COLOR_BGR2RGB);

	#segmentation mask
	rcnn = cv2.imread(os.path.join(data_path, filename.replace('_img','_maskDL')),0);
