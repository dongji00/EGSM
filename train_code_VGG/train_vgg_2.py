...
#Original Data
traindata =  AdobeDataAffineHR(csv_file='data_train/all.csv',data_config=data_config_train,transform=None)  #Write a dataloader function that can read the database provided by .csv file
...
net.load_state_dict(torch.load(model_dir + '/net_epoch_12_0.4579.pth')) #uncomment this if you are initializing your model
net.cuda()
torch.backends.cudnn.benchmark=True
print(net)
#summary(net.cuda(), input_size=((3,3,1,4), 256, 256), batch_size=4)


'''# 加载vgg模型
def load_model(model_name, model_dir):
	model = eval('models.%s(init_weights=False)' % model_name)
	path_format = os.path.join(model_dir, '%s-[0-9a-z]*.pth' % model_name)
	print(path_format)
...
vgg_model_dir = 'D:/keke/matting/mine-matting_0823/train_code_VGG/vgg_model/'
vgg = load_model('vgg16', vgg_model_dir)'''

#model = torch.load("D:/keke/matting/mine-matting_0823/train_code_VGG/vgg_model/vgg16-397923af.pth")
#print(vgg)
#torch.save(model.state_dict(),"D:/keke/matting/mine-matting_0823/train_code_VGG/vgg_model/vgg16-397923af.pth") # 保存参数

vgg_model = models.vgg16(pretrained=True)
print(vgg_model)
pretrained_dict = vgg_model.state_dict()
model_dict = Vgg16().state_dict()
#model_dict_alpha = Vgg16_alpha().state_dict()
vgg = Vgg16()
#vgg_alpha = Vgg16_alpha()
#init_vgg16(args.vgg_model_dir)
#model_dict = Vgg16.state_dict()
#pretrained_dict = vgg.load_state_dict(torch.load(os.path.join(args.vgg_model_dir)))
pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
#pretrained_dict_alpha = {k: v for k, v in pretrained_dict.items() if k in model_dict_alpha}
model_dict.update(pretrained_dict)
#model_dict_alpha.update(pretrained_dict_alpha)
vgg.load_state_dict(model_dict,strict=False)
#vgg_alpha.load_state_dict(model_dict,strict=False)
#vgg.load_state_dict(torch.load(os.path.join(args.vgg_model_dir)))
vgg.cuda()
#vgg_alpha.cuda()
#Loss
l1_loss=alpha_loss()
...
optimizer.load_state_dict(torch.load(model_dir + '/optim_epoch_12_0.4579.pth')) #uncomment this if you are initializing your model
...
for epoch in range(0,args.epoch):
	...
	for i,data in enumerate(train_loader):
		#Initiating
		write_step = open("write_step_train_adobe_vgg.txt", 'a')
		...
		features_alpha_pred = vgg_alpha(1,alpha_pred)
		f_xc_alpha = Variable(features_alpha_pred[1].data, requires_grad=False)
		features_alpha = vgg_alpha(alpha)
		#gram_style = [gram_matrix(y) for y in features_style]
		mse_loss = torch.nn.MSELoss()
		perceptual_loss_alpha = args.perceptual_weight * mse_loss(features_alpha[1], f_xc_alpha)
'''
		# load the pre-trained vgg-16 and extract features
