...
class AdobeDataAffineHR(Dataset):
	...
	def __getitem__(self,idx):
		try:
			...
			#random flip
			if np.random.random_sample() > 0.5:
				alpha = cv2.flip(alpha[0:2],1)
				fg = cv2.flip(fg[0:2],1)
				back = cv2.flip(back[0:2],1)
				image = cv2.flip(image[0:2],1)
...
# The first parameter represents the shape of the kernel, and there are three shapes to choose from, rectangles: MORPH_RECT; Cross form: MORPH_CROSS; Ellipse: MORPH_ELLIPSE; The second and third parameters are the size of the kernel and the location of the anchor points, respectively.
# Generally define a variable of type Mat to get the return value of the getStructuringElement function before calling erode and dilate functions:
# For anchor location, there is a default value Point (-1,-1), indicating that the anchor is at the center point. The element shape only depends on the anchor position; in other cases, the anchor point only affects the shift of the result of the morphological operation.
