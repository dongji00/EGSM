...
from train_preprocessing.data_loader import AdobeDataAffineHR
from train_preprocessing.functions import *
from train_preprocessing.networks import ResnetConditionHR, conv_init
from train_preprocessing.loss_functions import alpha_loss, compose_loss, alpha_gradient_loss
from torchsummary import summary

#CUDA
#-n train_adobe_fuzzy -bs 4 -res 256
...
parser.add_argument('-epoch', '--epoch', type=int, default=30,help='Maximum Epoch')
...
#Original Data
traindata =  AdobeDataAffineHR(csv_file='data_adobe_fuzzy/train_adobe_fuzzy.csv',data_config=data_config_train,transform=None)  #Write a dataloader function that can read the database provided by .csv file
...
net.load_state_dict(torch.load(model_dir + '/net_epoch_33_0.1852.pth'))
#net.load_state_dict(torch.load(model_dir + '/net_epoch_2_0.2543.pth')) #uncomment this if you are initializing your model
net.cuda()
torch.backends.cudnn.benchmark=True
print(net)
summary(net.cuda(), input_size=((3,3,1,4), 512, 512), batch_size=1)
...
optimizer.load_state_dict(torch.load(model_dir + '/optim_epoch_33_0.1852.pth'))
#optimizer.load_state_dict(torch.load(model_dir + '/optim_epoch_2_0.2543.pth')) #uncomment this if you are initializing your model
...
for epoch in range(0,args.epoch):
	...
	for i,data in enumerate(train_loader):
		#Initiating
		write_step = open("write_step_train_adobe_fuzzy.txt", 'a')
		...
		if i % step == (step-1):
			...
			print('[%d, %5d] Total-loss:  %.4f Alpha-loss: %.4f Fg-loss: %.4f Comp-loss: %.4f Alpha-gradient-loss: %.4f Time-all: %.4f Time-fwbw: %.4f' % (epoch + 1, i + 1, netL/step, alL/step, fgL/step, fg_cL/step, al_fg_cL/step, elapse/step, elapse_run/step), file = write_step)
			netL, alL, fgL, fg_cL, al_fg_cL, elapse_run, elapse=0,0,0,0,0,0,0
